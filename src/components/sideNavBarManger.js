...
const SideNav = () => {
  ...
  const deletemessage = async () => {
    const del = await axios.post(`http://localhost:5000/deletmessage/${id}`)
    if (del.data === "done") {
      alert("Done")
    }
    else {
    ...
    <>
      <div className='sidenav'>
        <div className='inside-sidenav'>
          ...
          <NavLink className='sidenav-links d-flex' to={`/selectwinner/${id}`}>
            <div><GrAnnounce /></div><div style={{ paddingTop: '5px', paddingLeft: '14px', fontSize: '17px' }}>Select Winner</div>
          </NavLink>
